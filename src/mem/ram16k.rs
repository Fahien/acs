...
use crate::{mem::fast::Ram4k, Demux4Way, Mux4Way16, Signal, Signal14, Signal16, Time, Unit};

/// A Random Access Memory consisting of 16k 16-bit registers
#[derive(Default)]
pub struct Ram16k {
    time: Time,
    inp: Signal16,
...
impl Ram16k {
    ...
    pub fn sim(&mut self, inp: Signal16, load: Signal, address: Signal14) -> Signal16 {
        self.inp = inp;
        self.load = load;
...
impl Unit for Ram16k {
    fn tick(&mut self) {
        self.time += 1;
        let mux_signal = self.address.get(12..14).into();
        let ram_loads = self.demux.sim(self.load, mux_signal);
        let mut ram_outs = [Signal16::FALSE; 4];
        for i in 0..4 {

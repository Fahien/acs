...
use crate::{mem::fast::Ram64, Demux8Way, Mux8Way16, Signal, Signal16, Signal9, Time, Unit};

/// A Random Access Memory consisting of 512 16-bit registers
#[derive(Default)]
pub struct Ram512 {
    time: Time,
    inp: Signal16,
...
impl Ram512 {
    ...
    pub fn sim(&mut self, inp: Signal16, load: Signal, address: Signal9) -> Signal16 {
        self.inp = inp;
        self.load = load;
...
impl Unit for Ram512 {
    fn tick(&mut self) {
        self.time += 1;
        let mux_signal = self.address.get(6..9).into();

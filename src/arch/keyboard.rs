...
impl Keyboard {
    pub fn set(&mut self, keycode: sdl2::keyboard::Keycode) {
        self.out = to_ascii(keycode).into();
    }

    pub fn unset(&mut self, keycode: sdl2::keyboard::Keycode) {
        if self.out == to_ascii(keycode) {
            self.out = Signal16::FALSE;
        }
    }
}

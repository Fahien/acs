...
impl Unit for Alu {
    fn eval(&mut self) {
        // Zero or X
        let x_zero_mux_out = if self.zx.as_bool() {
            Signal16::FALSE
        } else {
            self.x
        };
        ...
        // Now not X
        let x_negx_mux_out = if self.nx.as_bool() {
            !x_zero_mux_out
        } else {
            x_zero_mux_out
        };
        ...
        // Zero or Y
        let y_zero_mux_out = if self.zy.as_bool() {
            Signal16::FALSE
        } else {
            self.y
        };
        ...
        // Now not Y
        let y_negy_mux_out = if self.ny.as_bool() {
            !y_zero_mux_out
        } else {
            y_zero_mux_out
        };
        ...
        let f_mux_out = if self.f.as_bool() {
            x_negx_mux_out + y_negy_mux_out
        } else {
            x_negx_mux_out & y_negy_mux_out
        };

        // Select between Out and !Out
        let o_nego_mux_out = if self.no.as_bool() {
            !f_mux_out
        } else {
            f_mux_out
        };
        ...
        self.zr = (o_nego_mux_out == Signal16::FALSE).into();
